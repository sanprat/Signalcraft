...
**App Shortcuts:**
The manifest includes 2 shortcuts for quick access:
- Dashboard
- Strategy Builder
...
- [ ] **Real-time signals** — Push notifications for strategy signals
...
| lightweight-charts | ^4.1 | TradingView-style charts (currently used) |
| KlineCharts | latest | TradingView-style charts (planned) |
...
Impact: Cannot fetch live data (historical data still available via parquet files)
Severity: Low (not needed for backtesting with historical data)
Fix: Rotate token if live data becomes needed again
```

### **KlineCharts v10 Integration Issue** ✅ FIXED (March 29, 2026)
```
Issue: KlineCharts v10 API is significantly different from lightweight-charts/v9
Status: ✅ RESOLVED - Build passes, chart renders correctly
Severity: Was HIGH - blocked Phase 3 (Results Visualization)

Root Causes Fixed:
  1. getBars params: v10 provides { callback, type, timestamp, symbol, period }
     - NOT { fromTimestamp, toTimestamp, qty } (v9 assumption)
  2. getBars callback: params.callback(data[], false) - boolean for "more data"
     - NOT (data[], { timestamp, noMore }) (v9 assumption)
  3. createOverlay takes ONE argument - points must be inside first arg object
     - Fixed: chart.createOverlay({ name: 'simpleAnnotation', points: [{ timestamp, value }], ... })
  4. Built-in 'simpleAnnotation' overlay used for trade markers (no custom registerOverlay needed)
  5. subscribeBar callback expects single KLineData, not array
  6. dispose() takes container ref, not chart object

Correct v10 API Pattern (Verified via Context7 + type definitions):
  - Initialize: const chart = init(container) // returns Nullable<Chart>
  - DataLoader: chart.setDataLoader({ getBars: ({ callback }) => callback(candles, false) })
  - Markers: chart.createOverlay({ name: 'simpleAnnotation', points: [{ timestamp, value }] })
  - Crosshair: chart.subscribeAction('onCrosshairChange', (data) => { ... })
  - Cleanup: dispose(containerRef.current!)

Files fixed: frontend/app/backtest/[id]/page.tsx
Commit: pending
```
...
| Backtest → Chart | ✅ Working | KlineCharts v10 API fixed, chart renders with trade markers |
| Trade Markers | ✅ Working | Green ▲ BUY / Red ▼ SELL annotations on chart |
