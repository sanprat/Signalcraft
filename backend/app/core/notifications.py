...
async def send_user_telegram_message(user_id: Optional[int], message: str):
    """
    ...
    If user_id is None, falls back to global notification.
    """
    ...
    if user_id is not None:
        try:
            ...
            logger.warning(
                f"Could not load Telegram credentials for user {user_id}: {e}"
            )
