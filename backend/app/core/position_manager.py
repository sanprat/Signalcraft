...
class PositionManager:
    ...
    async def _sync_positions_from_db(self):
        ...
        with get_db() as conn:
            ...
            cursor.close()
        logger.info(
            f"Position Manager: Synced {len(self.active_positions)} open positions."
        )

    async def handle_signal(self, strategy_data: dict, symbol: str, price: float):
        """Handle an entry signal from SignalMonitor."""
        logger.info(
            f"Position Manager: Handling signal for {symbol} at {price} (Strategy: {strategy_data['name']})"
        )

        # 1. Risk Check: Max trades per day
        if not await self._check_risk_limits(strategy_data):
            logger.warning(
                f"Risk limit reached for strategy {strategy_data['name']}. Skipping signal."
            )
            return
    ...
    async def _check_risk_limits(self, strategy_data: dict) -> bool:
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            today_start = datetime.now(self._IST).replace(
        ...
        if count >= max_trades:
            logger.warning(
                f"Trade limit ({max_trades}) reached for {strategy_data['name']}"
            )
            return False

        if float(today_realized_pnl) <= -max_loss:
            logger.warning(
                f"Daily loss limit (₹{max_loss}) reached for {strategy_data['name']}. Current: ₹{today_realized_pnl}"
            )
            user_id = strategy_data.get("user_id")
            if user_id is None:
                logger.warning(
                    f"Cannot send Telegram notification: user_id not found in strategy {strategy_data['id']}"
                )
            await send_user_telegram_message(
                user_id,
                f"🚨 *Risk Limit Reached*\nStrategy: {strategy_data['name']}\nReason: Daily loss limit of ₹{max_loss} hit.\nStopping for the day.",
            )
            return False
    ...
    async def open_position(self, strategy_data: dict, symbol: str, price: float):
        ...
        try:
            ...
            with get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT broker, status FROM live_strategies WHERE id = %s",
                    (strategy_data["id"],),
                )
                row = cursor.fetchone()
                broker = row[0]
            ...
            broker_order_id = (
                f"PAPER_ENTRY_{datetime.now(self._IST).strftime('%Y%m%d%H%M%S')}"
            )

            if not is_paper:
                logger.info(
                    f"Placing BUY order for {symbol}, qty={quantity} via {broker}..."
                )
                ...
                for attempt in range(retries):
                    try:
                        ...
                        if order_res and order_res.get("status") != "error":
                            ...
                            logger.warning(
                                f"Attempt {attempt + 1} failed checking status: {msg}"
                            )
                    ...
                    if attempt == retries - 1:
                        logger.error(
                            f"Order placement failed after {retries} attempts."
                        )
                        await send_user_telegram_message(
                ...
                broker_order_id = order_res.get("data", {}).get(
                    "orderId"
                ...
                logger.info(
                    f"PAPER TRADING: Simulating BUY order for {symbol}, qty={quantity}..."
                )
                pos_status = "OPEN"
            ...
            if pos_status == "OPEN":
                logger.info(
                    f"✅ Position opened! ID: {pos_id}, Symbol: {symbol}, Price: {price}, SL: {stoploss:.2f}, TP: {target:.2f}"
                )
                await send_user_telegram_message(
                    strategy_data.get("user_id"),
                    f"🚀 *Position Opened*\nStrategy: {strategy_data['name']}\nSymbol: {symbol}\nPrice: ₹{price}\nQty: {quantity}\nSL: ₹{stoploss:.2f}\nTP: ₹{target:.2f}",
                ...
                logger.info(
                    f"⏳ Position PENDING! ID: {pos_id}, Symbol: {symbol}, waiting for broker execution..."
                )
                await send_user_telegram_message(
                    strategy_data.get("user_id"),
    ...
    async def _sync_broker_positions(self):
        ...
        with get_db() as conn:
            ...
            for pos in open_positions:
                ...
                if sid not in strat_info_cache:
                    cursor.execute(
                        "SELECT broker, status, user_id FROM live_strategies WHERE id = %s",
                        (sid,),
                    )
                    row = cursor.fetchone()
                    if row:
        ...
        for pos in open_positions:
            ...
            if qty == 0:
                ...
                logger.warning(
                    f"⚠️ Position {pos['symbol']} was manually closed on the broker app! Auto-closing in Pytrader."
                )
                ...
                exit_price = (
                    quote.get("ltp", 0.0) if quote else float(pos["entry_price"])
                )

                pnl = (exit_price - float(pos["entry_price"])) * int(pos["quantity"])
                pnl_pct = (
                    (pnl / (float(pos["entry_price"]) * int(pos["quantity"]))) * 100
                ...
                user_id = strat_info.get("user_id")
                if user_id is None:
                    logger.warning(
                        f"Cannot send Telegram notification: user_id not found for strategy {pos['live_strategy_id']}"
                    )
                await send_user_telegram_message(
                    user_id,
                    f"⚠️ *Manual Intervention Detected*\nSymbol: {pos['symbol']}\nPorted to CLOSED because net quantity on {strat_info['broker']} is 0.",
                )

    async def _reconcile_pending_orders(self):
        """Check status of PENDING orders with the broker."""
        pending_positions = [
            p for p in self.active_positions if p["status"] == "PENDING"
        ]
        ...
        with get_db() as conn:
            ...
            for pos in pending_positions:
                ...
                if sid not in strat_info_cache:
                    cursor.execute(
                        "SELECT broker, exit_conditions, name, user_id FROM live_strategies WHERE id = %s",
                        (sid,),
                    )
                    row = cursor.fetchone()
                    if row:
        ...
        for pos in pending_positions:
            ...
            if st == "COMPLETE":
                ...
                logger.info(
                    f"✅ Order {broker_order_id} COMPLETE! Entry: {entry_price}, SL: {stoploss}, TP: {target}"
                )
                user_id = strat_info.get("user_id")
                if user_id is None:
                    logger.warning(
                ...
                with get_db() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "UPDATE positions SET status = 'REJECTED' WHERE id = %s",
                        (pos["id"],),
                    )
                    cursor.execute("COMMIT")
                    cursor.close()
    ...
    async def close_position(self, position: dict, exit_price: float, reason: str):
        ...
        try:
            ...
            broker_exit_id = (
                f"PAPER_EXIT_{datetime.now(self._IST).strftime('%Y%m%d%H%M%S')}"
            )

            if not is_paper:
                logger.info(
                    f"Closing position {position['id']} ({position['symbol']}) at {exit_price} due to {reason}..."
                )
                ...
                for attempt in range(retries):
                    try:
                        ...
                        if order_res and order_res.get("status") != "error":
                            ...
                            logger.warning(
                                f"Exit attempt {attempt + 1} failed checking status: {msg}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Exit attempt {attempt + 1} raised exception: {e}"
                        )

                    if attempt == retries - 1:
                        logger.error(
                            f"Failed to close position {position['id']} after {retries} attempts."
                        )
                        if user_id is None:
                            logger.warning(
                                f"Cannot send Telegram notification: user_id not found for position {position['id']}"
                            )
                        await send_user_telegram_message(
                            user_id,
                            f"🚨 *URGENT: Failed to Close Position*\nSymbol: {position['symbol']}\nAttempts: {retries}\nPlease check broker APP manually!",
                        )
                        return
                    await asyncio.sleep(2)

                broker_exit_id = order_res.get("data", {}).get(
                    "orderId"
                ) or order_res.get("order_id")
            else:
                logger.info(
                    f"PAPER TRADING: Simulating SELL order for {position['symbol']} at {exit_price} (Reason: {reason})..."
                )
            ...
            logger.info(
                f"✅ Position {position['id']} closed! PnL: {pnl:.2f} ({pnl_pct:.2f}%)"
            )
