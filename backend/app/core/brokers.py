...
class ShoonyaAdapter(BrokerAdapter):
    def __init__(self, creds=None):
        ...
        try:
            ...
            totp_secret = creds.get("totp_secret") or os.environ.get(
                "SHOONYA_TOTP_SECRET", ""
            )
...
class FlattradeAdapter(BrokerAdapter):
    ...
    def __init__(self, creds=None):
        ...
        try:
            ...
            totp_secret = creds.get("totp_secret") or os.environ.get(
                "FLATTRADE_TOTP_SECRET", ""
            )
            totp = pyotp.TOTP(totp_secret).now() if totp_secret else ""
            self.api.login(
                userid=creds.get("userid") or os.environ.get("FLATTRADE_USER_ID", ""),
                password=creds.get("password")
                or os.environ.get("FLATTRADE_PASSWORD", ""),
                twoFA=totp,
                vendor_code=creds.get("vendor_code")
                or os.environ.get("FLATTRADE_VENDOR_CODE", ""),
                api_secret=creds.get("api_secret")
                or os.environ.get("FLATTRADE_API_SECRET", ""),
                imei="",
            )
        except Exception as e:
    ...
    def cancel_order(self, order_id):
        return bool(self.api.cancel_order(orderno=order_id)) if self.api else False
...
class ZerodhaAdapter(BrokerAdapter):
    def __init__(self, creds=None):
        ...
        try:
            ...
            access_token = creds.get("access_token") or os.environ.get(
...
class DhanAdapter(BrokerAdapter):
    ...
    def __init__(self, creds=None):
        ...
        self.access_token = creds.get("access_token") or os.environ.get(
            "DHAN_ACCESS_TOKEN", ""
        )
        self.headers = {
    ...
    def place_order(self, symbol, exchange, action, qty, price=0, order_type="MARKET"):
        ...
        r = requests.post(
            f"{self.BASE}/orders", json=payload, headers=self.headers, timeout=10
        )
        return r.json()
    ...
    def cancel_order(self, order_id):
        r = requests.delete(
            f"{self.BASE}/orders/{order_id}", headers=self.headers, timeout=10
        )
    ...
    def get_order_status(self, order_id):
        try:
            ...
            if r.status_code == 200:
                ...
                if "TRADED" in st or "COMPLETE" in st:
                    st = "COMPLETE"
...
def get_adapter(broker: str, user_id: int = None) -> BrokerAdapter:
    ...
    if cache_key not in _adapters:
        ...
        match broker.lower():
            case "shoonya":
                _adapters[cache_key] = ShoonyaAdapter(creds)
            case "flattrade":
                _adapters[cache_key] = FlattradeAdapter(creds)
            case "zerodha":
                _adapters[cache_key] = ZerodhaAdapter(creds)
            case "dhan":
                _adapters[cache_key] = DhanAdapter(creds)
            case _:
