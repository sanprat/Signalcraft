#!/usr/bin/env python3
"""
download_nifty50_test.py — Download 1-minute data for NIFTY 50 stocks (2021-2026 Test)

Fetches 1-min intraday OHLCV from Jan 2021 to current date for top 50 highly liquid stocks
to test if Dhan API serves deep historical intraday data for these.
Saves to data/candles/NIFTY500/{SYMBOL}/1min_test.parquet (so we don't mess up main files).
...
# Date range - TESTING FROM 2021 TO SEE IF DHAN PERMITS IT
DEFAULT_START = date(2021, 1, 1)
DEFAULT_END = date.today()

# The typical 50 large caps
NIFTY_50_SYMBOLS = [
...
def download_stock_1min(
    ...
    # Use a different filename so we don't mess up the pipeline ones
    output_path = BASE_DIR / symbol / "1min_test.parquet"
    stats = {
        "symbol": symbol,
        "requests": 0,
        "candles": 0,
    ...
    if not existing_df.empty:
        ...
        log.info(f"Existing test data found, latest: {latest}")
        start = max(start, latest.date() + timedelta(days=1))
        if start > end:
            log.info(f"{symbol}: Test file already up to date")
            return stats
    ...
    if all_candles:
        ...
        log.info(
            f"✓ {symbol}: Saved {stats['candles']} candles (total in test file: {total_rows})"
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Download 1-minute data for NIFTY 50 (from Jan 2021) as test",
    ...
    if not symbols:
        log.error("No Nifty 50 stocks with valid mappings found!")
        return
    ...
    log.info("=" * 70)
    log.info("NIFTY 50 1-Minute Data Test (2021-2026)")
    log.info("=" * 70)
    ...
    elapsed = time.time() - start_time
    log.info("\n" + "=" * 70)
    log.info("DOWNLOAD COMPLETE - TEST SUMMARY")
    log.info("=" * 70)
