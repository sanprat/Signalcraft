...
Run daily at 6:00 AM (before market opens):
0 6 * * * cd /path/to/project && python3 data-scripts/auto_refresh_dhan_token.py --force >> /var/log/dhan_token_refresh.log 2>&1

Or run twice daily for extra safety:
0 6,18 * * * cd /path/to/project && python3 data-scripts/auto_refresh_dhan_token.py --force >> /var/log/dhan_token_refresh.log 2>&1
"""
...
PROACTIVE_REFRESH_HOURS = 20.0  # Refresh when there are 20 hours or less remaining
...
def generate_new_token():
    ...
    if current_token:
        try:
            ...
            renew_headers = {"access-token": current_token, "dhanClientId": client_id}
            resp = requests.get(
                "https://api.dhan.co/v2/RenewToken", headers=renew_headers, timeout=15
            )
...
def refresh_token(force=False):
    ...
    if expiry_ts and not force:
        ...
        if hours_remaining > PROACTIVE_REFRESH_HOURS:
            logger.info(
                f"Token still valid ({hours_remaining:.1f}h > {PROACTIVE_REFRESH_HOURS}h), skipping refresh"
            )
            logger.info("Use --force to refresh anyway")
            return True
    elif force:
        logger.info("FORCE MODE: Refreshing token regardless of current status")
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force refresh regardless of token status"
    )
